...
// zkVM patch: usleep is a no-op. No need to sleep in deterministic execution.
...
// zkVM patch: return fixed wall time. No SYSCALL needed.
...
// zkVM patch: return monotonically increasing fake nanotime using a global
// counter at address 0x7efff000. Increments by 1ms (1000000 ns) per call.
// Avoids SYSCALL and the expensive MULU that follows it in the original.
// This gives the Go scheduler the illusion of time passing so it doesn't
// spin-poll excessively.
TEXT runtime·nanotime1(SB),NOSPLIT,$0-8
	MOVW	$0x7efff000, R3
	MOVW	0(R3), R4		// load low word
	MOVW	4(R3), R5		// load high word
	ADDU	$1000000, R4, R6	// low += 1ms
	SGTU	R4, R6, R7		// carry if wrapped
	ADDU	R7, R5, R8		// high += carry
	MOVW	R6, 0(R3)		// store low
	MOVW	R8, 4(R3)		// store high
	MOVW	$ret+0(FP), R3
#ifdef GOARCH_mips
	MOVW	R6, 4(R3)		// low word
	MOVW	R8, 0(R3)		// high word
#else
	MOVW	R6, 0(R3)		// low word
	MOVW	R8, 4(R3)		// high word
#endif
	RET
